...
/// <summary>
/// Decoder for CCITT Group 3 (T.4) Fax compression.
/// Based on UTIF.js _decodeG3 implementation.
/// Supports both 1D and 2D encoding modes.
/// </summary>
internal static class TiffFax3Decoder
{
    /// <summary>
    /// Decodes Group 3 compressed data.
    /// </summary>
    ...
    {
        return Decode(data, width, height, twoDimensional: false);
    }

    /// <summary>
    /// Decodes Group 3 compressed data with specified encoding mode.
    /// </summary>
    /// <param name="data">Compressed data.</param>
    /// <param name="width">Image width in pixels.</param>
    /// <param name="height">Image height in pixels.</param>
    /// <param name="twoDimensional">True if 2D encoding is used.</param>
    /// <returns>Decompressed bilevel data (1 bit per pixel, packed into bytes).</returns>
    public static byte[] Decode(byte[] data, int width, int height, bool twoDimensional)
    {
        ...
        {
            // Find b1 and b2 reference positions for 2D mode
            ...
            if (is1D)
            {
                // 1D mode - simple run-length decoding
            ...
            // Check for EOL (End Of Line) marker
            if (word.EndsWith("000000000001"))
            {
                if (y >= 0)
                {
                ...
                if (twoDimensional)
                {
                    // Read the next bit to determine 1D/2D mode for next line
                    is1D = GetBit(data, bitPos) == 1;
                    bitPos++;
                }
        ...
        // Write last line if not already written
        if (line.Count == width && y >= 0 && y < height)
        {
            WriteBits(line, output, y * bytesPerRow * 8, width);
        }
